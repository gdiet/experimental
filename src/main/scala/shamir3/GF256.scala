...
extension (b: B)
  @scala.annotation.targetName("multiplyWith")
  def *(c: B): B = if b.is0 || c.is0 then B.zero else EXP(~LOG(b) + ~LOG(c))
  @scala.annotation.targetName("divideBy")
  def /(c: B): B = b * EXP(255 - ~LOG(c))

private val LOG: Array[B] = B(java.util.HexFormat.of.parseHex(SLOG))
private val EXP: Array[B] = B(java.util.HexFormat.of.parseHex(SEXP))
