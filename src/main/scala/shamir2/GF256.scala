...
extension[T](a: Array[T])
  def apply(b: Byte): T = a(b.uInt)

extension (b: Byte)
  def uInt: Int = java.lang.Byte.toUnsignedInt(b)
  def #+(c: Byte): Byte = (b ^ c).toByte
  def #-(c: Byte): Byte = (b ^ c).toByte
  def #*(c: Byte): Byte = if b == 0 || c == 0 then 0 else EXP(LOG(b).uInt + LOG(c).uInt)
  def #/(c: Byte): Byte = b #* EXP(255 - LOG(c).uInt)

private val LOG: Array[Byte] = java.util.HexFormat.of.parseHex(SLOG)
private val EXP: Array[Byte] = java.util.HexFormat.of.parseHex(SEXP)

private def SLOG: String =
